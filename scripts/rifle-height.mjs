...
const OUT = path.resolve(process.env.HOME, 'unreal/_work/rifle-heights'); fs.mkdirSync(OUT, { recursive: true })
...
try {
  ...
  await page.waitForFunction(()=>{ const v=window.gameClient.simulator.viewmodel; return v && v.ready && v.spec && v.spec.name==='Rifle' }, {timeout:20000}).catch(()=>{})
  await pump(1200)
